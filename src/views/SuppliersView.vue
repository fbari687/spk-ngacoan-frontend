...
const handleSubmit = async () => {
  ...
  try {
    ...
    serverError.value = err.response?.data?.message || 'Terjadi kesalahan sistem saat menyimpan data supplier.';
  } finally {
...
const handleDelete = async (id) => {
  if (confirm('Apakah Anda yakin ingin menghapus supplier ini? Peringatan: Data perhitungan yang melibatkan supplier ini di masa lalu mungkin akan terpengaruh.')) {
    try {
...
<template>
  <div class="space-y-6 animate-in fade-in duration-300">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <h2 class="text-2xl md:text-3xl font-bold tracking-tight text-zinc-900">Mitra Pemasok (Supplier)</h2>
      ...
      <Button v-if="authStore.isOwner" @click="openAddDialog" class="w-full sm:w-auto bg-zinc-900 hover:bg-zinc-800 gap-2 px-4 py-5 text-sm cursor-pointer">
    ...
    <div v-else-if="suppliers.length === 0" class="h-64 flex flex-col items-center justify-center space-y-4 bg-white border border-zinc-200 rounded-xl shadow-sm p-6 text-center">
      ...
      <div>
        ...
        <p class="text-sm text-zinc-500 mt-1">Klik tombol di atas untuk mendaftarkan kandidat supplier pertama Anda.</p>
      </div>
    </div>
      ...
      <Table>
        <TableHeader class="bg-zinc-50">
          <TableRow>
            ...
            <TableHead v-if="authStore.isOwner" class="w-28 text-right font-semibold text-zinc-700">Aksi</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          <TableRow v-for="item in suppliers" :key="item.id" class="hover:bg-zinc-50/50 transition-colors">
            ...
            <TableCell class="text-zinc-600">
              <div class="flex items-start gap-2">
                ...
                <span class="truncate max-w-[300px]">{{ item.address || '-' }}</span>
              </div>
            </TableCell>
            <TableCell class="text-zinc-600">
    ...
    <Dialog v-model:open="isDialogOpen">
      <DialogContent class="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Masukkan informasi detail terkait pemasok. Pastikan nomor kontak aktif untuk keperluan operasional.
          </DialogDescription>
        </DialogHeader>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4 py-4">
          ...
          <div class="space-y-2">
            ...
            <Input id="name" v-model="form.name" placeholder="Contoh: PT. Sumber Pangan" required class="focus-visible:ring-zinc-900" />
          </div>
          ...
          <div class="space-y-2">
            ...
            <Input id="phone" type="tel" v-model="form.phone" placeholder="Contoh: 081234567890" class="focus-visible:ring-zinc-900" />
          </div>
